...
The 3 primary data sets are already cleaned and saved as .csv files.

The structure of the 3 data sets are shown below.

#### **investment.csv:**

```{r, echo=FALSE}
variable <- c("category","meta_cat","group_num","year","gross_inv")
...
invest_dic <- data.frame(variable, description) %>%
  mutate(class = map(investment,typeof)) %>% 
  select(variable,class,description)
kable(invest_dic,
      align = "lll","pipe")
```
...
```{r, echo=FALSE}
variable <- c("category","meta_cat","group_num","year","gross_inv_chain")
description <- c(
```
...
```{r, echo=FALSE}
variable <- c("category","meta_cat","group_num","year","gross_inv_ipd")
description <- c("Category of investment",
                 "Group category of investment",
                 "Group number of investment",
                 "Year of investment",
...
ipd_dic <- data.frame(variable, description) %>%
  mutate(class = map(ipd,typeof)) %>% 
  select(variable,class,description)
kable(ipd_dic,
      align = "lll","pipe")
```
